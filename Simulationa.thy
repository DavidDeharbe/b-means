...
definition simulation :: "'st rel \<Rightarrow> ('st, 'ev) LTSa rel" where
  "simulation r \<equiv> { (l,l') | l l'.
     alpha l = alpha l'
   \<and> (\<forall>s \<in> init l. \<exists>s' \<in> init l'. (s, s') \<in> r)
...
lemma simulation_identity : "(Id::('st,'ev) LTSa rel) \<subseteq> simulation (Id::'st rel)" 
...
*}
sledgehammer_params[verbose, provers="z3 cvc4 e spass", timeout=300]
...
proof -
  ...
  next
    fix t
    assume "t \<in> trans l" "src t \<in> init l" "lbl t \<in> alpha l"
    with sim obtain s' t' where
      "(src t, s') \<in> r" "s' \<in> init l'"
      "t' \<in> trans l'" "src t' = s'" "(t,t') \<in> sim_transition r" "lbl t' \<in> alpha l'"
  ...
  next
    fix t ts
    assume t: "t \<in> trans l" "src t = dst (last ts)" "lbl t \<in> alpha l"
    ...
    with sim t obtain t' where
      t': "t' \<in> trans l'" "src t' = dst (last ts')" "(t,t') \<in> sim_transition r" "lbl t' \<in> alpha l'"
      sledgehammer
