...
text {* 
  ...
  i.e. every action taken by $R$, in response to an event that is considered acceptable by $A$
  shall be simulated by a corresponding action in $A$.

  It is called a gluing invariant and must be such that $R$ cannot diverge in situations where
  $A$ does not diverge. But $R$ may well be more "robust" and not diverge in a state where $A$
  would diverge. Of course, in that cases, the behavior of $R$ is not constrained. In the case
  where $A$ does not diverge, then $R$ must exhibit a behavior that corresponds to one of the
  possible behaviors of $A$.
  ...
  We say that an event @{text "e"} is \emph{accepted} in a state @{text "s"} of a LTS if @{text "(s, e)"}
  does not belong to the divergences of the LTS (not formalized).
...
text {*
  Notice that this classic notion of transition is symmetric, since it does not consider
  possible divergences. Next function is an attempt to rephrase the simulation between 
  transitions taking into account a set of divergences. 
*}

definition sim_div_transition :: "'st rel \<Rightarrow> ('st * 'ev) set \<Rightarrow> ('st, 'ev) Tr rel" where
  "sim_div_transition r d \<equiv> 
    { (tc,ta) | tc ta. 
      (src tc, src ta) \<in> r \<and> lbl tc = lbl ta \<and>
      ((src ta, lbl ta) \<in> d \<or> 
       (src ta, lbl ta) \<notin> d \<and> (dst tc, dst ta) \<in> r) }"
...
definition simulation :: "'st rel \<Rightarrow> ('st, 'ev) LTSDiv rel" where
  "simulation r \<equiv> { (l,l') | l l'.
   ...
   \<and> (\<forall>s s'. (s, s') \<in> r \<longrightarrow>
        (\<forall>e . 
          e \<in> div_state l' s' \<or>
         (e \<notin> div_state l s \<and> 
              (\<forall>t \<in> trans l. src t = s \<longrightarrow> lbl t = e \<longrightarrow>
                   (\<exists>t' \<in> trans l'. src t' = s' \<and> lbl t' = e \<and> (t,t') \<in> sim_transition r))))) }"

text {*
  The definition of simulation with divergences between LTSes can also be phrased using the
  simulation with divergences on transitions:
*}

definition simulation_div :: "'st rel \<Rightarrow> ('st, 'ev) LTSDiv rel" where
  "simulation_div r \<equiv> { (l,l') | l l'.
     (\<forall>s \<in> init l. \<exists>s' \<in> init l'. (s, s') \<in> r)
   \<and> (\<forall>s s'. (s, s') \<in> r \<longrightarrow>
        (\<forall>t \<in> trans l. src t = s \<longrightarrow>
           (\<exists>t' \<in> trans l'. src t' = s' \<and> lbl t' = lbl t \<and> 
              (t,t') \<in> sim_div_transition r (divergences l')))) }"
...
theorem sim_run:
  assumes 
    sim: "(l,l') \<in> simulation r" 
  and 
    ts: "ts \<in> runs l"
  and
    safe: "\<forall> t . t \<in> set ts \<and> t \<notin> div_trans l'"
...
proof -
  ...
  next
    ...
    with sim obtain t' where
      "(src t, src t') \<in> r" "src t' \<in> init l'"
      "t' \<notin> div_trans l'" "(t,t') \<in> sim_transition r"
      unfolding simulation_def sorry
  ...
  next
    ...
    with sim t obtain t' where
      t': "t' \<notin> div_trans l'" "src t' = dst (last ts')" "(t,t') \<in> sim_transition r"
      unfolding simulation_def sorry
...
lemma sim_traces:
  assumes sim: "(l,l') \<in> simulation r" and tev: "(t, ev) \<in> trace_div l"
  shows "\<exists> (t', ev') \<in> trace_div l'. t = t' \<and> ev' \<subseteq> ev"
sorry
