...
text {* 
  ...
  reachable states of a given LTSa @{text m}. 
*}

inductive_set states :: "('st, 'ev) LTSa \<Rightarrow> 'st set" 
  for l :: "('st, 'ev) LTSa" where
  base[elim!]: "s \<in> init l \<Longrightarrow> s \<in> states l"
| step[elim!]: "\<lbrakk> t \<in> trans l; src t \<in> states l; lbl t \<in> alpha l \<rbrakk> \<Longrightarrow> dst t \<in> states l"
...
definition successors :: "('st, 'ev) LTSa \<Rightarrow> 'st set \<Rightarrow> 'st set" where
  "successors l S \<equiv> { dst t | t . t \<in> trans l \<and> src t \<in> S \<and> lbl t \<in> alpha l }"
...
inductive_set runs :: "('st, 'ev) LTSa \<Rightarrow> ('st, 'ev) Run set"  
  for l :: "('st, 'ev) LTSa" where
  base: "[] \<in> runs l"
| start: "\<lbrakk> t \<in> trans l; lbl t \<in> alpha l; src t \<in> init l \<rbrakk> \<Longrightarrow> [t] \<in> runs l"
| step: "\<lbrakk> t \<in> trans l; lbl t \<in> alpha l; ts \<in> runs l; ts \<noteq> []; src t = dst (last ts) \<rbrakk> \<Longrightarrow> ts @ [t] \<in> runs l"
...
definition traces :: "('st, 'ev) LTSa \<Rightarrow> 'ev Trace set" where
  "traces l \<equiv> (map lbl) ` (runs l)"

lemma trace_in_alphabet : "\<lbrakk> tr \<in> traces l \<rbrakk> \<Longrightarrow> set tr \<subseteq> alpha l"
