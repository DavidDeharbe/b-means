...
definition
  outgoing_trans :: "('st, 'ev) LTSa \<Rightarrow> 'st \<Rightarrow> ('st, 'ev) Tr set"
where
  "outgoing_trans l s \<equiv> { t | t . t \<in> trans l \<and> src t = s \<and> lbl t \<in> alpha l}"
...
definition
  accepted_events :: "('st, 'ev) LTSa \<Rightarrow> 'st \<Rightarrow> 'ev set"
where
...
*}
definition simulation_B :: "'st rel \<Rightarrow> ('st, 'ev) LTSa rel" where
...
definition run_accepted_events :: "('st, 'ev) LTSa \<Rightarrow> ('st, 'ev) Run \<Rightarrow> 'ev set" where 
...
definition run_trace :: "('st, 'ev) LTSa \<Rightarrow> ('st, 'ev) Run \<Rightarrow> 'ev TrB" where 
...
definition traces_B :: "('st, 'ev) LTSa \<Rightarrow> 'ev TrB set" where
...
record ('st, 'ev) B_refinement =
  abstract :: "('st, 'ev) LTSa"     -- "the abstract component"
  concrete :: "('st, 'ev) LTSa"     -- "the concrete component "
...
definition refinement_id :: "('st, 'ev) LTSa \<Rightarrow> ('st, 'ev) B_refinement" where
